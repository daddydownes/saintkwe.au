(() => {
 ...
 let reduced=query.matches,index=0,running=false,paused=false,inspecting=false,muted=true,transition=0,last=0,playing=false;
 ...
 videos.forEach(v=>{v.preload='auto';v.playsInline=true;v.setAttribute('playsinline','');v.muted=true;v.defaultMuted=true;v.volume=.65;v.hidden=true;
  ...
  v.addEventListener('waiting',()=>{if(v!==current()||!running||paused||suspended||needsGesture||mediaFailed)return;playing=false;setLoading(true);$('media-status').textContent='Loading the preview…';});
  v.addEventListener('pause',()=>{
   const intentional=intentionalPauses.delete(v);
   if(v!==current())return;
   playing=false;
   if(intentional||!running||paused||suspended||segmentComplete||inspecting||document.hidden||v.ended)return;
   loadingSlow=false;needsGesture=true;mediaFailed=false;setLoading(false);updateSound();
   $('media-status').textContent='Playback paused. Tap Play Music to continue.';
  });
  v.addEventListener('timeupdate',()=>{if(v===current()&&running&&!paused&&!suspended&&!inspecting&&v.currentTime>=8)completeSegment();});
  v.addEventListener('ended',()=>{if(v!==current()||!running||paused||suspended||mediaFailed)return;if(inspecting){v.currentTime=0;play();}else completeSegment();});
  v.addEventListener('error',()=>{if(v!==current()||!running||suspended)return;showMediaError();});
 });
 ...
 function waitForNext(){
  ...
  if(nextLoadingTimer!=null)return;
  $('media-status').textContent='Loading the next preview…';
  const track=index;
  nextLoadingTimer=setTimeout(()=>{
   nextLoadingTimer=null;if(!running||paused||suspended||index!==track||nextReady())return;
   nextFallback=true;
   $('media-status').textContent='Continuing while the next preview loads…';
  },6000);
 }
 function completeSegment(){
  if(segmentComplete)return;
  if(index===tracks.length-1){finish();return;}
  segmentComplete=true;pauseMedia(current());
 }
 ...
 function warmNext(){
  const v=current();if(!running||paused||suspended||mediaFailed||navigator.connection?.saveData)return;
 ...
 function setLoading(value){
  ...
  if(loadingTimer!=null||loadingSlow)return;
  const track=index;
  loadingTimer=setTimeout(()=>{
   loadingTimer=null;
   if(!running||paused||suspended||playing||index!==track)return;
   loadingSlow=true;updateSound();
   $('media-status').textContent='The preview is taking longer to load. You can wait or tap Retry.';
  },15000);
 }
 function updateSound(){
  const label=mediaFailed||loadingSlow?'Retry':paused?'Resume':needsGesture?(muted?'Play':'Play sound'):muted?'Unmute':'Mute';
  $('sound').textContent=label;$('sound').setAttribute('aria-label',label);
  $('sound').setAttribute('aria-pressed',String(muted));
  const prompt=running&&!suspended&&!mediaFailed&&!loadingSlow&&(muted||needsGesture),focused=document.activeElement;
  $('sound').hidden=!running||suspended||prompt;
 ...
 function showMediaError(message='Preview unavailable. Tap Retry above or tap the video to watch on YouTube.'){panels[index].classList.remove('has-frame');playing=false;loadingSlow=false;mediaFailed=true;needsGesture=false;setLoading(false);updateSound();$('media-status').textContent=message;}
 ...
 async function play(){
  if(!running||paused||suspended)return;
  const v=current(),request=++playRequest;intentionalPauses.delete(v);v.muted=muted;
  try{
   await v.play();
   if(request!==playRequest||!running||paused||suspended)return;
   markPlaying(v);
  }catch(e){
   if(request!==playRequest||!running||paused||suspended||e.name==='AbortError')return;
   if(e.name!=='NotAllowedError'){showMediaError();return;}
   playing=false;loadingSlow=false;needsGesture=true;mediaFailed=false;setLoading(false);updateSound();
 ...
 function finish(){running=false;clearLoadingTimer();clearNextTimer();++playRequest;videos.forEach(pauseMedia);updateSound();$('journey').hidden=true;$('arrival').hidden=false;window.KweMedia?.loadWithin($('arrival'));$('replay').focus({preventScroll:true});}
 function suspend(){
  if(suspended)return;
  suspended=true;clearLoadingTimer();clearNextTimer();++playRequest;playing=false;loadingSlow=false;
  videos.forEach(pauseMedia);updateSound();
 }
 function resume(){
  if(!suspended)return;
  suspended=false;updateSound();
  if(!running||paused||document.hidden)return;
  if(segmentComplete){waitForNext();return;}
 ...
 $('sound').addEventListener('click',()=>{
  if(suspended)return;
  if(loadingSlow){loadingSlow=false;setLoading(true);updateSound();play();return;}
 ...
 document.addEventListener('visibilitychange',()=>{if(!running||suspended)return;if(document.hidden){resumeAfterVisibility=!paused;if(resumeAfterVisibility)pause(true);}else if(resumeAfterVisibility){resumeAfterVisibility=false;pause(false);}});
 window.addEventListener('pagehide',suspend);
 window.addEventListener('pageshow',event=>{if(event.persisted)resume();});
 document.addEventListener('keydown',e=>{if(e.key==='Escape'){if(window.KweTransitions)window.KweTransitions.navigate('/demo-v3/index.html#listen');else{suspend();location.href='/demo-v3/index.html#listen';}}});
